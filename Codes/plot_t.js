...
function getMonthlyData(stock) {
  ...
  var queryUrl = `https://www.alphavantage.co/query?function=TIME_SERIES_MONTHLY&symbol=${stock}&apikey=demo${apiKey}`;
  ...
  Object.entries(data["Monthly Time Series"]).forEach(([key, value]) => {
  dates.push(key);
  ...
  buildTable(dates, openingPrices, highPrices, lowPrices, closingPrices, volume);
});
